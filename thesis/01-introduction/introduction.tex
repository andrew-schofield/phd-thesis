...
\begin{itemize}
 \item Very simple rate law modelling in \textit{Pseudomonas} by \citet{Jin2003}.
 \item An alternative approach involved modelling respiration using ``P systems'' which are probabilistic models of events. This assigned a probability of each reaction happening, dependant on the state of the system and then iterated through a given set of steps evaluating probabilities and altering values based on the outcome by \citet{Cavaliere2006}. This approach to modelling was limited in that it was only predicting the quantities of 1 component in each of 2 ``compartments''; oxygen in the cell membrane and carbon dioxide in the thylakoid membrane (the model was developed using cyanobacteria).
 ...
 \item Modelling ETC in purple bacteria by \citet{Klamt2008}.
 \item Predicted respiration using simple kinetic model for marine bacteria by \citet{Aguiar-Gonzalez2012}.
 \item Modelling metabolic fluxes in \Nsm{} by \citet{Baart2007}
